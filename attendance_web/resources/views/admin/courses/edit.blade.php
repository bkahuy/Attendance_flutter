...
@section('content')
    <div class="section-title"><h2>Sửa môn học</h2></div>
    @component('components.card')
        <form method="post" action="{{ route('admin.courses.update',$course->id) }}" style="max-width:640px">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:10px">
                <div>
                    <label>Tín chỉ</label>
                    <input class="form-control" type="number" min="1" max="10" name="credits" value="{{ old('credits',$course->credits) }}" required>
                </div>
                <div>
                    ...
                    <select class="form-control" name="department_id">
                        ...
                        @foreach($departments as $d)
                            <option value="{{ $d->id }}" @selected(old('department_id',$course->department_id)==$d->id)>{{ $d->name }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
