...
@section('content')
    ...
    <div class="card" style="max-width:720px">
        <form method="post" action="{{ route('admin.students.update',$student->id) }}">
            ...
            <input class="form-control" name="student_code" value="{{ old('student_code',$student->student_code) }}" required>
                ...
                <div>
                    ...
                    <select class="form-control" name="class_name">
                        ...
                        @foreach($classes as $c)
                            <option value="{{ $c }}" @selected(old('class_name',$student->class_name)==$c)>{{ $c }}</option>
                        @endforeach
                    </select>
                </div>
                <div>
                    ...
                    <select class="form-control" name="faculty">
                        ...
                        @foreach($faculties as $f)
                            <option value="{{ $f }}" @selected(old('faculty',$student->faculty)==$f)>{{ $f }}</option>
                        @endforeach
                    </select>
                </div>
            </div>
